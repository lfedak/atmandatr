% ODEs for sixth experimental condition
% Liz Fedak
% Created: 10/13/19
% Updated: 10/20/20


function ydot = exp6_ODEs(t,y,par)
...
ATM_f = kATM*ATM./(jATM + LDSB + LDSBA + LCDSB + LCDSBA + ATRpP + ATRpF);
...
dLERdt       = kMRN*MRNs*(LDSB + LCDSB) + kPL*(LPD + FP) - LER.*ATR_g - (kBRCA + kSSA)*LER; % CHANGED
dLERAdt      = LER.*ATR_g + kPL*(LPDA + FPA) + rA*(kMRN*MRNs*(LCDSBA + LDSBA) - (kBRCA + kSSA)*LERA); % CHANGED
dLEERdt      = kBRCA*LER - LEER.*ATR_g - kHR*LEER; % CHANGED
dLEERAdt     = LEER.*ATR_g + rA*(kBRCA*LERA - kHR*LEERA); % CHANGED
