% ODEs for experimental condition 4
% Liz Fedak
% Created: 10/13/19
% Updated: 10/20/20


function ydot = exp4_ODEs(t,y,par)
...
LMDS      = y(28); % Multiply damaged sites on DNA that has not yet been replicated
LMDSR     = y(29); % Multiply damaged sites on replicated DNA
FMDS      = y(30); % Replication forks stalled on MDS
FMDSA     = y(31); % ATR-bound replication forks stalled on MDS
...
ATM   = ATM_tot - ATMp;
ATR   = ATR_tot - ATRp;
ATR_g = kATR*XPAs*ATR./(jATR + FP + FPA + FMDS + FMDSA + LPD + LPDA + LDSB + LDSBA + LCDSB + LCDSBA + LER + LERA + LEER + LEERA);
...
dLCDSBdt     = -(kSNHEJ + kMRN*MRNs)*LCDSB - LCDSB.*(ATM_f + ATR_g) + kCDSB*(LMDS + LMDSR); % CHANGED
dLCDSBAdt    = LCDSB.*(ATM_f + ATR_g) -rA*(kSNHEJ + kMRN*MRNs)*LCDSBA;
dLERdt       = kMRN*MRNs*(LDSB + LCDSB) + kPL*(LPD + FP + FMDS) - LER.*(ATM_f + ATR_g) - (kBRCA + kSSA)*LER; % CHANGED
dLERAdt      = LER.*(ATM_f + ATR_g) + kPL*(LPDA + FPA + FMDSA) + rA*(kMRN*MRNs*(LCDSBA + LDSBA) - (kBRCA + kSSA)*LERA); % CHANGED
...
dATRpFdt    = (FMDS + ktop*rA*FMDSA + FP + ktop*rA*FPA).*ATR_g - (rA*kTLS + kPL + kdATR)*ATRpF;

% NEW EQUATIONS

dLMDSdt     = -kCDSB*LMDS - kMR*LMDS - rho.*LMDS;
dLMDSRdt    = -kCDSB*LMDSR - kMR*LMDSR + kTLS*(FMDS + rA*FMDSA);
dFMDSdt     = rho.*LMDS - kPL*FMDS - ATR_g.*FMDS - kTLS*FMDS;
dFMDSAdt    = -kPL*FMDSA + ATR_g.*FMDS - kTLS*FMDSA;
